...
const Button = ({
    ...
    const outlineClasses = {
        ...
        primary:
            'text-primary-600 bg-transparent border border-primary-600 hover:bg-primary-100 focus:ring-primary-500',
        secondary:
            'text-secondary-600 bg-transparent border border-secondary-600 hover:bg-secondary-100 focus:ring-secondary-500',
        success:
            'text-green-600 bg-transparent border border-green-600 hover:bg-green-100 focus:ring-green-500',
        danger: 'text-red-600 bg-transparent border border-red-600 hover:bg-red-100 focus:ring-red-500',
        warning:
            'text-yellow-400 bg-transparent border border-yellow-400 hover:bg-yellow-100 focus:ring-yellow-300',
    ...
    const prependIconElement = prependIcon ? (
        <i className={`${prependIcon} ${iconClasses} me-2`} />
    ) : null;

    const appendIconElement = appendIcon ? (
        <i className={`${appendIcon} ${iconClasses} ms-2`} />
    ) : null;

    if (as === 'a' && to !== undefined) {
        return (
